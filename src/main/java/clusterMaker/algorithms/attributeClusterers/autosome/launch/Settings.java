...
public class Settings implements Serializable {
    ...
    public String getSettings() {
        ...
        if (!doSM && !noMapping) {
            ...
            if (doCart)
                sb.append("SOM error dampener (theta)\t" + som_theta + "\n"); //error dampening factor; error is divided by theta
            if (doCart)
                sb.append("SOM error surface exponent\t" + som_DEScale + "\n"); //set error to this number as an exponent
        }
        if (!noMapping) {
            if (doCart)
                sb.append("Cartogram resolution\t" + de_resolution + "x" + de_resolution + "\n"); //diffusion resolution: must be a power of 2
        ...
        if (benchmark)
            sb.append("No. Known Clusters\t" + known_clusters + "\n");//set number of known clusters a priori
        if (doKmeans) sb.append("Kmeans");
        if (doHierarchical)
            sb.append("Agglomerative Clustering Metric\t" + ((hierarchical_choice == 1) ? "single" : (hierarchical_choice == 2) ? "complete" : (hierarchical_choice == 3) ? "average" : "Ward's" + "\n")); //1=single,2=complete,3=average,4=wards linkage distance metric
