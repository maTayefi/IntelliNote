...
import clusterMaker.algorithms.attributeClusterers.autosome.clustering.mst.MSTCluster;
import clusterMaker.algorithms.attributeClusterers.autosome.cluststruct.clusterRun;
import clusterMaker.algorithms.attributeClusterers.autosome.cluststruct.dataItem;
import clusterMaker.algorithms.attributeClusterers.autosome.cluststruct.getClusters;
import clusterMaker.algorithms.attributeClusterers.autosome.cluststruct.sortCluster;
import clusterMaker.algorithms.attributeClusterers.autosome.mapping.cartogram.DEC;
import clusterMaker.algorithms.attributeClusterers.autosome.mapping.som.SOM;
...
public class Run {
        ...
        public void run() {
            ...
            for (; start < end; start++) {
                try {
                    ...
                    if (!s.batch)
                        if (s.ensemble_runs > 1 && Math.floor((runCount++) % ((double) (s.ensemble_runs) / 10)) == 0) {
                            //    System.out.print("*");
    ...
    public dataItem[] getSumSqr(Settings s) {
        ...
        if (s.sumSqrRows) {
            for (int i = 0; i < s.input.length; i++) {
                float sumSqr = 0;
                for (int j = 0; j < s.input[i].getValues().length; j++)
                    sumSqr += (s.input[i].getValues()[j] * s.input[i].getValues()[j]);
        ...
        if (s.sumSqrCol) {
            for (int i = 0; i < s.input[0].getValues().length; i++) {
                float sumSqr = 0;
                for (int j = 0; j < s.input.length; j++)
                    sumSqr += (s.input[j].getValues()[i] * s.input[j].getValues()[i]);
                double sqrRoot = Math.sqrt(1 / (double) sumSqr);
                if (Double.isNaN(sqrRoot)) sqrRoot = 0;
    ...
    public class launchSplash implements Runnable {
        public void run() {
            ...
            ImageIcon icon = new ImageIcon(getClass().getResource("/imgs/autosomebanner.jpg"));
