        ...
        for (k = 0; k <= lz; k++) {
            for (i = 0; i < lx; i++) {
                for (j = 0; j < ly; j++) {
                    ...
                    if (rho_0[k].array[i][j] == 100 || rho_0[k].array[i][j] == Double.MAX_VALUE || Double.isInfinite(rho_0[k].array[i][j]))
                        rho_0[k].array[i][j] = avgdens;
...
//  logger.finest("Gaussian blur ...\n");
        ...
        for (k = 0; k < lz; k++)
            for (i = 0; i < lx; i++)
                for (j = 0; j < ly; j++)
                    if (rho_0[k].array[i][j] < minpop)
                        minpop = rho_0[k].array[i][j];
        for (k = 0; k < lz; k++)
            for (i = 0; i < lx; i++)
                for (j = 0; j < ly; j++)
                    if (rho_0[k].array[i][j] > maxpop)
...
// with Newton-Raphson. Returns TRUE after sufficient convergence.
    ...
    private boolean newt2(float h, float[] retxyAppr, float xguess, float yguess, float zguess,
        ...
        for (i = 1; i <= IMAX; i++) {
            ...
            if ((deltax * deltax + deltay * deltay + deltaz * deltaz) < TOLX) {
                retxyAppr[0] = xappr;
                retxyAppr[1] = yappr;
                retxyAppr[2] = zappr;
                return true;
