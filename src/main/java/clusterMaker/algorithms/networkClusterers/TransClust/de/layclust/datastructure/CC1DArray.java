...
public class CC1DArray implements ICCEdges {
     ...
     *
     * @param node_i The value of the first edge node.
     * @param node_j The value of the second edge node.
     ...
     *
     * @param node_i The value of the first edge node.
     * @param node_j The value of the seciond edge node.
...
//            cc2d.normalise();
        ...
        for (int i = 0; i < size; i++) {
            for (int j = i + 1; j < size; j++) {
                if (Math.abs(edgeCostArray[pos[i] + (j - i) - 1]) > max)
                    max = Math.abs(edgeCostArray[pos[i] + (j - i) - 1]);
